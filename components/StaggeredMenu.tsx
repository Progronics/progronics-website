...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  return (
    <div
      className={`sm-scope z-40 ${isFixed ? `fixed top-0 left-0 w-screen ${open ? "h-screen" : "h-[60px]"}  overflow-hidden` : 'w-full h-full'}`}
    >
      <div
      ...
      >
        ...
        <header
          className="staggered-menu-header absolute top-0 left-0 w-full h-[50px] flex items-center justify-between p-[2em] bg-transparent pointer-events-none z-20 backdrop-blur-xl border-b border-border/20"
          ...
          >
            ...
            >
              <span ref={textInnerRef} className="sm-toggle-textInner flex flex-col leading-none">
                {textLines.map((l, i) => (
                  <span className={`sm-toggle-line block h-[1em] leading-none  ${open ? 'text-black' : 'text-[#e9e9ef]'
                    }`} key={i}>
                    {l}
                  </span>
                ))}
            ...
            <span
              ref={iconRef}
              className={`sm-icon relative w-[14px] h-[14px] shrink-0 inline-flex items-center justify-center [will-change:transform] ${open ? 'text-black' : 'text-[#e9e9ef]'
                    }`}
              aria-hidden="true"
            >
              <span
