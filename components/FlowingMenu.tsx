...
const MenuItem: React.FC<MenuItemProps> = ({ link, text, description }) => {
  ...
  const repeatedMarqueeContent = React.useMemo(() => {
    return Array.from({ length: 4 }).map((_, idx) => (
      <React.Fragment key={idx}>
        <span className="text-[#060010] uppercase font-normal text-[3vh] leading-[1.2] p-[1vh_1vw_0]">{description}</span>
        {/* <div
  ...
  return (
    <div className="flex-1 relative overflow-hidden text-center shadow-[0_-1px_0_0_#fff]" ref={itemRef}>
      ...
      <div
        className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none bg-[#f6f6f6] translate-y-[101%]"
        ref={marqueeRef}
      >
